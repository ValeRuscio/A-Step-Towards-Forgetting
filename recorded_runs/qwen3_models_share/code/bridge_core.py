...
Uses the supplied bridge_replay.py core for exact association replay.
"""
...
import numpy as np
import torch
from bridge_replay import (Config, Engine, read, load, make_data, digest, minibatches,
...
def run(s):
    ...
    with open(out/'queue.lock','a') as lock:
        ...
        with Progress(out) as progress:
            ...
            design=dict(settings=s,inputs=fingerprints(s),code={p.name:sha256_file(p) for p in [Path(__file__),Path(__file__).with_name('bridge_replay.py')]},
...
def _process(output):
    ...
    try:
        ...
        if not any(Path(x).name=='bridge_core.py' for x in cmd):return None
...
def _incompatible(out,s):
    ...
    if (out/'design.json').exists():
        ...
        code={p.name:sha256_file(p) for p in [Path(__file__),Path(__file__).with_name('bridge_replay.py')]}
        import transformers
        versions=dict(torch=torch.__version__,numpy=np.__version__,transformers=transformers.__version__)
...
def night_attention_arrays(e,row,layers):
    ...
    for layer in layers:
        ...
        q,k=rotate(q),rotate(k);scores=np.einsum("hid,hjd->hij",q,k)/math.sqrt(dim);scores=np.where(np.tri(length,dtype=bool),scores,-np.inf)
        P=np.exp(scores-scores.max(-1,keepdims=True));P/=P.sum(-1,keepdims=True)
        Y=np.einsum("hij,hjd->hid",P,v).transpose(1,0,2).reshape(length,-1);native=captures[(layer,'out')]
...
def night_run(s):
    ...
    with open(out/'queue.lock','a') as lock:
        ...
        with Progress(out) as progress:
            ...
            design=dict(settings=s,inputs=night_inputs(s),code={p.name:sha256_file(p) for p in [Path(__file__),Path(__file__).with_name('bridge_replay.py')]},versions=dict(torch=torch.__version__,numpy=np.__version__,transformers=transformers.__version__))
