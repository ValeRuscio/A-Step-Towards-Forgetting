...
def unavailable_models(models):
    ...
    for item in models:
        ...
        kind=item.get('architecture')
        symbols={'qwen3':['Qwen3Config','Qwen3ForCausalLM'],'qwen2':['Qwen2Config','Qwen2ForCausalLM'],'llama':['LlamaConfig','LlamaForCausalLM']}.get(kind,[])
        try:
            for name in symbols:getattr(transformers,name)
        except (AttributeError,ImportError,RuntimeError) as exc:
            blocked.append(dict(name=item['name'],repo=item['repo'],reason=str(exc),transformers=transformers.__version__,action='Required architecture unavailable; model remains pending without changing the environment.'))
    return blocked
