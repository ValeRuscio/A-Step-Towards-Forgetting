...
def test():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        pair=s.f.read(out/'paired_comparison.json');assert len(pair['cases'])==4 and len(pair['comparisons'])==2
        ...
        assert (out/'paired_models_share.zip').exists()
        ...
        print('PASS paired four-case pipeline, fixed predictors, data matching, paths, acquisition/outcome report, export/resume and immutable predictor check')
