...
def default_settings(source,output):
    ...
    return dict(source=str(source),output=str(output),device='cuda:0',threads=8,seeds=[1],events=[21,47],geometry_texts=[],
                ...
                extended=dict(enabled=True,null_repeats=8,ntk_enabled=True,ntk_mode='auto',ntk_prompts_per_domain=4,
...
def validate(s):
    ...
    if ext['ntk_prompts_per_domain']>c.entities:raise ValueError('NTK panel exceeds available entities')
...
def show(output,full=False):
    ...
    for name in ['structure_overview.png','weight_change_overview.png','ntk_overview.png','ph_rmt_frame_overview.png','ph_rmt_example.png']:
