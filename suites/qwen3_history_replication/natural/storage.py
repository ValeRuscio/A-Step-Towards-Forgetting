...
class Control:
 ...
 def pulse(self,**kw):
  if 'job' in kw and kw['job']!=self.fields.get('job'):self.fields={k:v for k,v in self.fields.items() if k in ['completed_jobs','total_jobs','completed_branches','total_branches']}
  if kw.get('phase')!=self.fields.get('phase'):
   for k in ['alpha','split','epsilon','completed_prompts','population_size']:self.fields.pop(k,None)
