...
def collect(e,grads,losses,cap,ages,previous,floor=1e-12,ctl=None):
    ...
    """
    rows={};age_rows={};age_norms={};reconstruction_sq=0.;moment_sq=0.;history_residual_sq=0.;history_sq=0.
    for n in e.params:
        if ctl:ctl.check();ctl.pulse(phase='CPU cross-time and age contractions',parameter=n)
        ...
        for group in [layer(n),'GLOBAL']:
            an=age_norms.setdefault(group,{k:0. for k in age_h})
            for k,v in age_h.items():an[k]+=inner(v,v)
    ...
    for (pop,comp,ch,group),v in rows.items():
        ...
        output.append(v)
    ar=[]
    for (p,c,l),v in age_rows.items():
        gsq=rows[(p,c,'h',l)]['g_squared']
        ar.append(dict(population=p,component=c,layer=l,projections=v,norms={k:math.sqrt(x) for k,x in age_norms[l].items()},cosines={k:cosine(v[k],gsq,x,floor) for k,x in age_norms[l].items()}))
