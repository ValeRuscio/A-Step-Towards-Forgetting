...
def defaults(olmo_source,previous,output=None):
    return dict(version=VERSION,olmo_source=str(Path(olmo_source).resolve()),previous=str(Path(previous).resolve()),
        output=str(Path(output or Path.cwd()/'runs/qwen3_natural_v1').resolve()),device='cuda:0',threads=8,hours=11.5,reserve_minutes=10,
        ...
        models=[dict(name='qwen3_06b_base',repo='Qwen/Qwen3-0.6B-Base',revision='main',enabled=True,family='qwen3_06b',variant='base',architecture='qwen3')])
...
def analyze_cases(s,frozen):
    ...
    lines=['QWEN3 NATURAL FORGETTING — identical plain-text prompts; original OLMo predictors reused without fitting','Lead 1 = advance warning from preceding pre-update vectors. Lead 0 = current-gradient-informed diagnostic.','Prediction threshold: A_test increase > 0.05 KL; all steps retained. Metrics are per seed, not independent-update confidence intervals.']
...
def _export(output):
    root=Path(output);dest=root/'qwen3_models_share.zip';tmp=dest.with_suffix('.tmp')
