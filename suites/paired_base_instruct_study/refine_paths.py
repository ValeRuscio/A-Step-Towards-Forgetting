...
def adaptive_integral(evaluate,initial_points,atol,rtol,max_points=257,max_depth=12,convergence_keys=None):
    ...
    keys=list(next(iter(cache.values()))['projections'])
    convergence_keys=convergence_keys or ['delta']
    def f(x):
        x=float(x)
        if x not in cache:
    ...
    def assess(a,b,depth):
        ...
        vals={key:integral(key) for key in keys}
        ...
        for key in convergence_keys:
            coarse_key=(b-a)/6*(rr[0]['projections'][key]+4*rr[2]['projections'][key]+rr[4]['projections'][key])
            errors[key]=abs(vals[key]-coarse_key);tolerances[key]=atol*(b-a)+rtol*abs(vals[key])
        score=max([abs(closure)/max(tol,1e-30)]+[errors[k]/max(tolerances[k],1e-30) for k in convergence_keys])
        return dict(a=a,b=b,depth=depth,contributions=vals,loss_change=change,closure_error=closure,nested_difference=difference,
            component_nested_errors=errors,component_tolerances=tolerances,tolerance=tol,passed=score<=1,score=score)
    leaves=[];capped=False
    ...
    total={k:sum(l['contributions'][k] for l in leaves) for k in keys}
