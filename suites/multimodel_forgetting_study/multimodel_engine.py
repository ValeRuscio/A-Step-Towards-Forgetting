"""Full-parameter FP32 adapters: original OLMo, GPT-NeoX/Pythia, Qwen3 base.
...
class MultiEngine(Engine):
    def __init__(self,c):
        ...
        if self.kind not in {'olmo','gpt_neox','qwen3'}:raise ValueError('Unsupported model architecture: '+self.kind)
...
def install():
    ...
    p.Engine=MultiEngine
    def layer_name(name):
        match=re.match(r'(?:model|gpt_neox)\.layers\.(\d+)\.',name)
        return match.group(1) if match else 'embedding_readout_other'
    f.layer_name=layer_name


def unavailable_models(models):
    """Defer Qwen when unavailable; never block unrelated Pythia work."""
    import transformers
    blocked=[]
    for item in models:
        if not item['enabled'] or 'qwen3' not in item['repo'].lower():continue
        try:
            getattr(transformers,'Qwen3Config');getattr(transformers,'Qwen3ForCausalLM')
        except (AttributeError,ImportError,RuntimeError) as exc:
            blocked.append(dict(name=item['name'],repo=item['repo'],reason=str(exc),transformers=transformers.__version__,
                action='Qwen3 support unavailable. Pythia work continues in the unchanged environment; Qwen remains pending.'))
    return blocked
