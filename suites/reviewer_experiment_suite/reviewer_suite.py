...
class Control:
    ...
    def pulse(self,**kw):
        if kw.get('phase')!=self.fields.get('phase'):
            for key in ['alpha','layer','condition','corner','split']:
...
def trajectory(e,data,s,seed,condition,anchor,root,con,ctl,frozen,case):
    ...
    if policy.startswith('random_'):
        source=policy[len('random_'):];reference=get(con,case+'/'+source,'step')
        if len(reference)!=s['b_steps']:raise RuntimeError('Matched random schedule requires completed alarm-policy trajectory')
        count=sum(bool(r['action']) for r in reference);rng=random.Random(seed*1009+(1 if source=='projection' else 2));random_steps=set(rng.sample(list(range(10,s['b_steps']+1)),count))
        put(con,job,0,'random_schedule',dict(steps=sorted(random_steps),matched_to=source,count=count,independent_of_test_losses=True))
