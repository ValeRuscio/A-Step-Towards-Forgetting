...
The default runs the **trajectory and within-update path experiment**, for seed 1 and actual B updates 20 and 21. It records the states immediately before and after those updates. It does not infer the intervening trajectory from distant endpoints. Change `SETTINGS["trajectory"]["steps"]` for denser measurements. All steps must be within the saved source B schedule, with a preceding saved fork.
